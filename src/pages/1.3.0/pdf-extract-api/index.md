...
**1.3.0 What is Extract?**

[The PDF Extract API (included with the PDF Services API)](src/pages/pdf-extract-api/extract-api.md) is a cloud-based web service that uses Adobe’s Sensei AI technology to automatically extract content and structural information from PDF documents – native or scanned – and to output it in a structured JSON format. The service extracts text, complex tables, and figures as follows:
...
The PDF Extract API can be embedded into any application using the [PDFServices SDK](src/pages/pdf-services-api/index.md) for Node.js, Python, .NET and Java. [Start a free trial that includes up to 1,000 free Document transactions or runs for 6 months, whichever occurs earlier.](https://www.adobe.com/go/dcsdks_credentials)
