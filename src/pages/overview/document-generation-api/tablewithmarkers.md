...
- The content between these markers is then dynamically populated by iterating over the array of objects corresponding to the tag(ARRAY_FIELD) specified in `table-start` marker.
...
Create both the tables with their respective table markers. In the above example, the **school** tag acts as the array
...
- Array Field of `table-start` in the inner table should be the nested field of array field used in `table-start` marker of the outer table.
