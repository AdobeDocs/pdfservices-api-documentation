---
title: Java | Quickstart | PDF Extract API
description: |
  Walk through a simple scenario using PDF Extract API in Java to extract content and report on major headers.
...
Our application will take a PDF, `Adobe Extract API Sample.pdf` (included as part of the Java sample project), and extract its contents. The results will be saved as a ZIP file, `ExtractTextInfoFromPDF.zip`. We will then parse the results from the ZIP and print out the text of any `H1` headers found in the PDF.
...
we define what PDF will be extracted. (The source `Adobe Extract API Sample.pdf` is included with the Java sample project.) In a real application, these values would typically be dynamic.
