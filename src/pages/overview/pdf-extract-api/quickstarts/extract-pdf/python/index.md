---
title: Python | Quickstart | PDF Extract API
description: |
  Walk through a simple scenario using PDF Extract API in Python to extract content and report on major headers.
...
Our application will take a PDF, `Adobe Extract API Sample.pdf` (included as part of the Python sample project), and extract its contents. The results will be saved as a ZIP file, `ExtractTextInfoFromPDF.zip`. We will then parse the results from the ZIP and print out the text of any `H1` headers found in the PDF.
