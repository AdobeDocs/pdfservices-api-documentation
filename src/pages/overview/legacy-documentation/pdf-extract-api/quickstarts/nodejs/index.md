...
description: |
  ...
  on major headers using Node.js.
...
Our application will take a PDF, `Adobe Extract API Sample.pdf` (included with the Node.js sample project), and extract its contents. The results will be saved as a ZIP file, `ExtractTextInfoFromPDF.zip`. We will then parse the results from the ZIP and print out the text of any `H1` headers found in the PDF.
...
This defines what our output ZIP will be and optionally deletes it if it already exists. Then we define what PDF will be extracted. (The source `Adobe Extract API Sample.pdf` is included with the Node.js sample project.) In a real application, these values would typically be dynamic. 
